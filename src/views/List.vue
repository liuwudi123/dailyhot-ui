            ...
            <n-list hoverable clickable style="width: 100%">
              ...
              >
                ...
                <div class="text">
                  ...
                  />
                  <n-button
                    v-if="listType === 'github' && !item.translated"
                    size="tiny"
                    secondary
                    type="primary"
                    style="margin-top: 8px; width: fit-content"
                    :loading="translatingId === (item.id || item.title)"
...
const handleTranslate = async (item) => {
  if (!item.desc) return;
  translatingId.value = item.id || item.title;
  const translated = await translateText(item.desc);
